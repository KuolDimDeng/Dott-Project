...
# Version0066_updated_solution_stack.sh
# Version: 1.0.0
# Date: 2025-05-22
# Author: AI Assistant
# Purpose: Create a final simplified deployment with correct solution stack parameter
...
echo -e "${BLUE}===== DottApps Updated Solution Stack Deployment (v1.0.0) =====${NC}"
...
# Get available solution stacks and use the latest
echo -e "${BLUE}Getting available Docker solution stacks...${NC}"
SOLUTION_STACKS=$(aws elasticbeanstalk list-available-solution-stacks --query "SolutionStacks[?contains(@, 'Docker')]" --output text)

# Choose the latest Amazon Linux 2 Docker solution stack
SOLUTION_STACK=$(echo "$SOLUTION_STACKS" | tr '\t' '\n' | grep "Amazon Linux 2" | head -n 1)

if [ -z "$SOLUTION_STACK" ]; then
    echo -e "${RED}Error: No Docker solution stack found. Using fallback.${NC}"
    SOLUTION_STACK="64bit Amazon Linux 2 v4.1.2 running Docker"
fi
...
# Display available solution stacks
echo -e "${BLUE}Available Docker solution stacks:${NC}"
echo "$SOLUTION_STACKS" | tr '\t' '\n' | nl

echo -e "${BLUE}Using solution stack: $SOLUTION_STACK${NC}"
...
if [[ "$RUN_DEPLOY" =~ ^[Yy]$ ]]; then
    ...
    echo -e "${BLUE}Solution Stack: $SOLUTION_STACK${NC}"
    ...
    if [ -z "$ENV_EXISTS" ]; then
        ...
        # Create the environment with minimal configuration and solution stack
