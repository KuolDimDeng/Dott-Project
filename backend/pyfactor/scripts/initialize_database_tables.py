...
def initialize_database_tables(db_name, force=False):
    ...
    if check_tables_exist(db_name):
        ...
        if not force:
            ...
            logger.warning("Force flag set. Proceeding with migrations despite existing tables.")
...
def main():
    ...
    if initialize_database_tables(db_name, force=args.force):
