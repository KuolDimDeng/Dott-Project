...
# Version 0084: Fixed Docker Deployment
# 
# CRITICAL: Previous deployment failed due to missing Dockerfile and sed syntax error
# This script ensures Docker configuration files are included with proper error handling
#
# Date: 2025-05-29
# Purpose: Deploy OAuth API endpoints with proper Docker configuration
# Issue: Missing Docker files and sed command syntax error

set -e

echo "=== Version 0084: Fixed Docker Deployment ==="
echo "⚠️  ISSUE: Previous deployment failed due to missing Docker configuration files"
echo "🔍 Current directory: $(pwd)"

# Application name and version
APP_NAME="Dott"
VERSION_LABEL="oauth-api-docker-fixed-v0084-$(date +%Y%m%d-%H%M%S)"
...
# Fix requirements conflicts (using portable method)
echo "🔧 Fixing dependency conflicts in requirements-eb.txt..."
if grep -q "psycopg2-binary==2.9.6" "$DEPLOYMENT_DIR/requirements-eb.txt"; then
    # Use a more portable sed approach
    sed 's/psycopg2-binary==2.9.6/psycopg2-binary==2.9.9/g' "$DEPLOYMENT_DIR/requirements-eb.txt" > "$DEPLOYMENT_DIR/requirements-eb.txt.tmp"
    mv "$DEPLOYMENT_DIR/requirements-eb.txt.tmp" "$DEPLOYMENT_DIR/requirements-eb.txt"
    echo "✅ Fixed psycopg2-binary version conflict (2.9.6 → 2.9.9)"
else
    echo "✅ No psycopg2-binary version conflict to fix"
fi
...
aws elasticbeanstalk create-application-version \
    ...
    --description "Fixed Docker OAuth API deployment - Version 0084 with Dockerfile and corrected dependencies"
...
echo "✅ Version 0084 deployment completed!"
echo ""
...
echo "   ✅ Fixed psycopg2-binary version conflict (2.9.6 → 2.9.9)"
echo "   ✅ Used portable sed syntax to avoid command errors"
...
echo "   • No more 'missing Dockerfile' errors"
