#!/usr/bin/env python3
"""
Efficient script to populate default county filing information for remaining US counties
Uses bulk operations and smaller batches to avoid timeouts
"""
...
def get_filing_day_from_deadline(deadline):
    """Extract numeric filing day from deadline string"""
    if '20th' in deadline:
        return 20
...
@transaction.atomic
def populate_county_filing_batch(counties_batch, state_info):
    """Process a batch of counties efficiently using bulk operations"""
    counties_to_update = []
    
    for county_record in counties_batch:
        # Only update if filing instructions are empty (not already populated)
        if not county_record.county_filing_instructions:
            ...
            if not county_record.filing_day_of_month:
                county_record.filing_day_of_month = get_filing_day_from_deadline(state_info['default_deadline'])
            ...
            counties_to_update.append(county_record)
    
    # Bulk update
    if counties_to_update:
        GlobalSalesTaxRate.objects.bulk_update(counties_to_update, [
            'county_filing_instructions',
            'county_filing_website', 
            'county_filing_deadline',
            'county_payment_methods',
            'manually_verified',
            'manual_notes',
            'filing_frequency',
            'filing_day_of_month',
...
def populate_remaining_county_filing_info():
    print("Populating Default County Filing Information for Remaining Counties")
    print("Using efficient batch processing to avoid timeouts")
    print("=" * 80)
    ...
    ).exclude(locality='').select_related()
    
    total_counties = counties_without_info.count()
    print(f"Found {total_counties} counties without filing information")
    
    if total_counties == 0:
        print("✅ All counties already have filing information!")
        return
    
    # Group by state for efficient processing
    counties_by_state = {}
    for county in counties_without_info:
        ...
        counties_by_state[state_code].append(county)
    
    print(f"Processing counties across {len(counties_by_state)} states...")
    
    for state_code, counties in counties_by_state.items():
        if state_code not in STATE_DEFAULT_INFO:
            print(f"⚠️  No state info for {state_code}, skipping {len(counties)} counties...")
            continue
            
        print(f"\n🏛️  Processing {state_code} - {len(counties)} counties...")
        states_processed += 1
        
        state_info = STATE_DEFAULT_INFO[state_code]
        batch_size = 50  # Process in smaller batches
        counties_updated_for_state = 0
        
        # Process counties in batches
        for i in range(0, len(counties), batch_size):
            batch = counties[i:i + batch_size]
            batch_updated = populate_county_filing_batch(batch, state_info)
            counties_updated_for_state += batch_updated
            total_counties_updated += batch_updated
            
            print(f"  📦 Batch {i//batch_size + 1}: Updated {batch_updated}/{len(batch)} counties")
        
        print(f"  ✅ {state_code} complete: {counties_updated_for_state} counties updated")
    
    print(f"\n🎉 COUNTY FILING INFO POPULATION COMPLETE!")
...
if __name__ == "__main__":
    populate_remaining_county_filing_info()