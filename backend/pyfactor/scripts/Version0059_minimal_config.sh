...
# Version0059_minimal_config.sh
# Version: 1.0.0
# Date: 2025-05-22
# Author: AI Assistant
# Purpose: Create a minimal configuration for Elastic Beanstalk deployment without HTTPS or other complex settings
...
echo -e "${BLUE}===== DottApps Minimal Configuration Generator (v1.0.0) =====${NC}"
...
if [ -f "$CONFIG_FILE" ]; then
    ...
    mkdir -p "$BACKUP_DIR"
    BACKUP_FILE="$BACKUP_DIR/DottAppsConfig_PRE_MINIMAL_$TIMESTAMP.bak.json"
...
echo -e "${YELLOW}Note: This is a minimal configuration without HTTPS${NC}"
...
if [ -f "$REGISTRY_FILE" ]; then
    ...
    if ! grep -q "Version0059_minimal_config.sh" "$REGISTRY_FILE"; then
...
| Version0059_minimal_config.sh | 1.0.0 | Create minimal EB configuration | $(date +"%Y-%m-%d") | Not Run |" "$REGISTRY_FILE"
