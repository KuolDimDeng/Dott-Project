...
def modify_onboarding_migration():
    """Modify the problematic onboarding migration"""
    print("Handling onboarding migration issue...")
    
    # Create a fake migration for onboarding that won't fail
    onboarding_migrations_dir = os.path.join(parent_dir, 'onboarding/migrations')
    
    # Make sure migrations directory exists
    os.makedirs(onboarding_migrations_dir, exist_ok=True)
    
    # Create a 0002 migration that won't fail
    migration_path = os.path.join(onboarding_migrations_dir, '0002_remove_onboardingprogress_business_and_more.py')
...
class Migration(migrations.Migration):
    ...
    operations = [
        # Empty operations as we handled this directly in SQL
    ]
""")
    
    print("✅ Created fixed onboarding migration")
    return True
...
def mark_migrations_as_applied():
    ...
    sql = "INSERT INTO django_migrations (app, name, applied) VALUES (%s, %s, NOW()) ON CONFLICT DO NOTHING;"
...
def main():
    ...
    try:
        ...
        # Handle the onboarding migration issue
        if not modify_onboarding_migration():
            print("Failed to modify onboarding migration. Aborting.")
            return
