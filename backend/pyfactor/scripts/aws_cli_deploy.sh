...
# Use the specific package we created
BACKEND_DIR="$(pwd)"
LATEST_PACKAGE="$BACKEND_DIR/fixed-docker-eb-package.zip"
...
if aws elasticbeanstalk describe-environments --environment-names "$ENV_NAME" --application-name "$APP_NAME" | grep -q "\"Status\": \"Ready\"\|\"Status\": \"Updating\""; then
  ...
  echo -e "${BLUE}Using saved configuration template: DottConfiguration${NC}"
  aws elasticbeanstalk create-environment \
    --application-name "$APP_NAME" \
    --environment-name "$ENV_NAME" \
    --platform-arn "$PLATFORM_ARN" \
    --template-name "DottConfiguration" \
    --version-label "$VERSION_LABEL"
