"""
Django management command to create WhatsApp Business tables directly.
...
class Command(BaseCommand):
    help = 'Create WhatsApp Business database tables directly (bypasses migrations)'
        ...
        with transaction.atomic():
            with connection.cursor() as cursor:
                ...
                cursor.execute("""
                    ...
                    WHERE table_name = 'whatsapp_catalogs'
                """)
                
                if cursor.fetchone()[0] > 0 and not options['force']:
                    self.stdout.write(self.style.WARNING(
                        'Tables already exist. Use --force to recreate them.'
                    ))
                    return
                ...
                cursor.execute("""
                    ...
                    CREATE TABLE IF NOT EXISTS whatsapp_products (
                        ...
                        created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        catalog_id BIGINT NOT NULL REFERENCES whatsapp_catalogs(id) ON DELETE CASCADE,
                        linked_product_id BIGINT REFERENCES inventory_product(id) ON DELETE SET NULL
                    );
                    ...
                    CREATE TABLE IF NOT EXISTS whatsapp_order_items (
                        ...
                        product_id BIGINT NOT NULL REFERENCES whatsapp_products(id) ON DELETE CASCADE
                    );
                    ...
                    CREATE TABLE IF NOT EXISTS whatsapp_messages (
                        ...
                        created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        related_order_id UUID REFERENCES whatsapp_orders(id) ON DELETE SET NULL,
                        tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE
                    );
                    ...
                    CREATE TABLE IF NOT EXISTS whatsapp_analytics (
                        ...
                        updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
                        tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
                        CONSTRAINT whatsapp_analytics_tenant_date_unique UNIQUE (tenant_id, date)
                    );

                    -- Create indexes
                    CREATE INDEX IF NOT EXISTS idx_whatsapp_business_settings_tenant ON whatsapp_business_settings(tenant_id);
                    CREATE INDEX IF NOT EXISTS idx_whatsapp_catalogs_tenant ON whatsapp_catalogs(tenant_id);
                    CREATE INDEX IF NOT EXISTS idx_whatsapp_products_catalog ON whatsapp_products(catalog_id);
                    CREATE INDEX IF NOT EXISTS idx_whatsapp_orders_tenant ON whatsapp_orders(tenant_id);
                    CREATE INDEX IF NOT EXISTS idx_whatsapp_orders_status ON whatsapp_orders(order_status);
                    ...
                    CREATE INDEX IF NOT EXISTS idx_whatsapp_analytics_tenant_date ON whatsapp_analytics(tenant_id, date);
                """)
                ...
                self.stdout.write(self.style.SUCCESS(
                    'Successfully created WhatsApp Business tables!'
                ))