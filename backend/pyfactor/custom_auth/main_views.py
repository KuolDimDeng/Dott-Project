                ...
                try:
                    ...
                    try:
                        if hasattr(user, 'owned_tenant'):
                            ...
                            logger.info(f"[ACTIVATION] Found owned tenant {owned_tenant.id} for user {user.email}")
                    ...
                    if owned_tenant and not user.tenant:
                        user.tenant = owned_tenant
                        user.save(update_fields=['tenant'])
                        logger.info(f"[ACTIVATION] Linked user {user.email} to their owned tenant {owned_tenant.id}")
                        tenant = owned_tenant
                    else:
                        # Check for any tenant where user is the owner
                        ...
                        if tenant_by_owner:
                            ...
                            logger.info(f"[ACTIVATION] No existing tenant found, creating new tenant schema for user {user.email}")
                            tenant = create_tenant_schema_for_user(user)
                            logger.info(f"[ACTIVATION] Tenant schema created successfully: { tenant.id}")
                except Exception as e:
                    logger.error(f"[ACTIVATION] Failed to create tenant schema during activation: {str(e)}")
