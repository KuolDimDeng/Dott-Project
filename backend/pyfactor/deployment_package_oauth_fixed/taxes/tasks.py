...
def populate_tax_rates_for_country(country_code, country_name, tenant_id=None):
    ...
    """
    logger.info(f"Starting tax rate population for {country_name} ({country_code})")
    
    try:
        # Initialize Claude client
        from django.conf import settings
        client = anthropic.Anthropic(api_key=settings.CLAUDE_TAX_API_KEY)
        ...
        if start_idx != -1 and end_idx > start_idx:
            ...
            logger.info(f"Successfully {'created' if created else 'updated'} tax rate for {country_name}: "
        ...
        logger.error(f"Error populating tax rate for {country_name}: {str(e)}")
        ...
        GlobalSalesTaxRate.objects.update_or_create(
            ...
            defaults={
                ...
                'ai_source_notes': 'Failed to fetch rate - manual verification required',
                'ai_last_verified': timezone.now(),
                'effective_date': timezone.now().date(),
                'is_current': True,
            }
