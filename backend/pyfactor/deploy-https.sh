#!/bin/bash
# deploy_optimized_eb.sh - Main script for deploying to AWS Elastic Beanstalk
# Version: 1.0.0
# Updated: May 19, 2025
...
ENVIRONMENT_NAME="Dott-env-https"
REGION="us-east-1"
...
ENV_OPTIONS_FILE="${SCRIPT_DIR}/environment-options-https-simple.json"
MONITOR_DEPLOYMENT=false

# Print header
echo -e "${BLUE}=======================================${NC}"
echo -e "${BLUE}      AWS Elastic Beanstalk Deployer   ${NC}"
echo -e "${BLUE}=======================================${NC}"
...
PACKAGE_OUTPUT=$(${SCRIPT_DIR}/scripts/create_optimized_docker_package.sh)
if [ $? -ne 0 ]; then
...
if [ "$ENV_EXISTS" = true ]; then
    ...
    if [ -f "$ENV_OPTIONS_FILE" ]; then
        echo -e "${BLUE}Applying environment options from ${ENV_OPTIONS_FILE}...${NC}"
