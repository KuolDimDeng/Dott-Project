...
echo -e "${BLUE}Finding the latest deployment package...${NC}"
BACKEND_DIR=$(dirname "$(pwd)")

if [[ "$OSTYPE" == "darwin"* ]]; then
    # macOS version (BSD find)
    LATEST_PACKAGE=$(find "$BACKEND_DIR" -name "complete-eb-package-*.zip" -type f -exec stat -f "%m %N" {} \; | sort -nr | head -1 | cut -d' ' -f2-)
    ...
    LATEST_PACKAGE=$(find "$BACKEND_DIR" -name "complete-eb-package-*.zip" -type f -printf "%T@ %p\n" | sort -nr | head -1 | cut -d' ' -f2-)
    if [ -z "$LATEST_PACKAGE" ]; then
...
if [ -z "$LATEST_PACKAGE" ]; then
    ...
    echo -e "${YELLOW}  node ./Version0040_create_complete_package.js${NC}"
    exit 1
fi
...
# Step 4: Check if the environment exists
