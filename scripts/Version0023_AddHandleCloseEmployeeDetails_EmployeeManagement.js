/**
 * Version0023_AddHandleCloseEmployeeDetails_EmployeeManagement.js
 * 
...
if (!updatedContent.includes('const handleCloseEmployeeDetails')) {
  ...
  const handleCloseEmployeeDetails = () => {
    ...
    if (!isEditing) {
      ...
      // If we can't find either function, add it after the state declarations
      const stateDeclarationLocation = updatedContent.indexOf('const [showEmployeeDetails, setShowEmployeeDetails] = useState(false);');
      
      if (stateDeclarationLocation !== -1) {
        // Find the end of the state declaration line
        const endOfStateDeclaration = updatedContent.indexOf('\n', stateDeclarationLocation) + 1;
        
        // Add the new function after state declaration
        const handleCloseEmployeeDetailsFunction = `
  ...
  const handleCloseEmployeeDetails = () => {
    ...
    if (!isEditing) {
      setSelectedEmployee(null);
        ...
        updatedContent = 
          updatedContent.substring(0, endOfStateDeclaration) + 
          handleCloseEmployeeDetailsFunction + 
          updatedContent.substring(endOfStateDeclaration);
        
        console.log('Added handleCloseEmployeeDetails function after state declaration');
      } else {
...
try {
  ...
  if (!scriptRegistryContent.includes('Version0023_AddHandleCloseEmployeeDetails_EmployeeManagement.js')) {
    const today = new Date().toISOString().split('T')[0];
    const newEntry = `| Version0023_AddHandleCloseEmployeeDetails_EmployeeManagement.js | Fixes handleCloseEmployeeDetails not defined error | ${today} | Completed |\n`;
