/**
 * Version0027_remove_test_tenant_prevention_layout.js
...
async function executeScript() {
  ...
  try {
    ...
    // Remove the large inline test-tenant prevention script
    updatedContent = updatedContent.replace(
      /\/\* IMMEDIATE TEST-TENANT PREVENTION[\s\S]*?console\.log\('✅ Test-tenant prevention applied inline'\);[\s\S]*?\}\s*`\s*\}\s*\/>/g,
      '/* Clean tenant initialization - test-tenant prevention removed */\n        <script dangerouslySetInnerHTML={{\n          __html: `' + CLEAN_TENANT_SCRIPT + '`\n        }} />'
    );
    
    // Remove duplicate test-tenant prevention script
    updatedContent = updatedContent.replace(
      /\/\* IMMEDIATE TEST-TENANT PREVENTION - HIGHEST PRIORITY \*\/[\s\S]*?console\.log\('✅ Test-tenant prevention applied inline'\);[\s\S]*?\}\s*`\s*\}\s*\/>/g,
      ''
    );
