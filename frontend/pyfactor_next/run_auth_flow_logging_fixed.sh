...
# Auth0 Authentication Flow Comprehensive Logging Script (Fixed version)
# Created: 2025-06-07
# Purpose: Implement detailed logging throughout the auth flow to diagnose 500 errors

echo "=== Starting Auth0 Authentication Flow Logging Implementation ==="
echo "This script will add comprehensive logging to identify auth0 login 500 errors"

# 1. First, create the comprehensive auth flow logging script with correct paths
cat > frontend/pyfactor_next/scripts/Version0140_add_comprehensive_auth_flow_logging_fixed.mjs << 'EOF'
// Version0140_add_comprehensive_auth_flow_logging_fixed.mjs
// Adds comprehensive logging throughout the authentication flow with corrected paths
// Created: 2025-06-07
...
const authRoutePath = 'frontend/pyfactor_next/src/app/api/auth/login/route.js';
const auth0RoutePath = 'frontend/pyfactor_next/src/app/api/auth/[...auth0]/route.js';
const callbackRoutePath = 'frontend/pyfactor_next/src/app/api/auth/callback/route.js';
const auth0ConfigPath = 'frontend/pyfactor_next/src/config/auth0.js';
const middlewarePath = 'frontend/pyfactor_next/src/middleware.js';
...
function createDocumentation() {
  const docPath = 'frontend/pyfactor_next/scripts/AUTH_FLOW_LOGGING_DOCUMENTATION.md';
...
echo "Created comprehensive auth flow logging script with corrected paths"

# 2. Next, create the deployment script
cat > frontend/pyfactor_next/scripts/Version0141_commit_and_deploy_auth_flow_logging_fixed.mjs << 'EOF'
...
echo "Created deployment script with corrected paths"

# 3. Run the scripts
echo "Running Version0140_add_comprehensive_auth_flow_logging_fixed.mjs to add detailed logging..."
node frontend/pyfactor_next/scripts/Version0140_add_comprehensive_auth_flow_logging_fixed.mjs

echo "Running Version0141_commit_and_deploy_auth_flow_logging_fixed.mjs to commit and deploy changes..."
node frontend/pyfactor_next/scripts/Version0141_commit_and_deploy_auth_flow_logging_fixed.mjs

echo "=== Auth0 Authentication Flow Logging Implementation Complete ==="
echo "You can now check for detailed logs in the authentication flow to diagnose the 500 error."
echo "Look for log entries with prefixes like [AUTH0-LOGIN], [AUTH0-HANDLER], [AUTH0-CONFIG]"
