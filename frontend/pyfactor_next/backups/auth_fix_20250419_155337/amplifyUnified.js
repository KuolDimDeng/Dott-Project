...
// Create enhanced auth functions that ensure configuration first
const ensureConfigAndCall = async (authFunction, ...args) => {
  ...
  // Check if Amplify is configured
  if (!isAmplifyConfigured()) {
    logger.warn('[AmplifyUnified] Amplify not configured before auth operation, configuring now');
    configureAmplify();
    
    // Double-check configuration was successful
    if (!isAmplifyConfigured()) {
      throw new Error('Failed to configure Amplify for authentication operation');
    }
  }
  
  // Call the wrapped function
  return authFunction(...args);
};

// Enhanced versions of auth functions that ensure configuration first
