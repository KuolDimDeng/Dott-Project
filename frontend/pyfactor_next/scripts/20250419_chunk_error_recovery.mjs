...
// Configuration
const config = {
  dashboardPagePath: path.resolve(__dirname, '../frontend/pyfactor_next/src/app/dashboard/page.js'),
  dashboardLayoutPath: path.resolve(__dirname, '../frontend/pyfactor_next/src/app/dashboard/layout.js'),
  tenantDashboardPagePath: path.resolve(__dirname, '../frontend/pyfactor_next/src/app/[tenantId]/dashboard/page.js'),
  dashboardLoaderPath: path.resolve(__dirname, '../frontend/pyfactor_next/src/components/DashboardLoader.js'),
  providersPath: path.resolve(__dirname, '../frontend/pyfactor_next/src/app/providers.js'),
  dashboardResiliencePath: path.resolve(__dirname, '../frontend/pyfactor_next/src/app/dashboard/DASHBOARD_RESILIENCE.md'),
};
...
function updateDashboardLoader() {
  ...
  // Replace the existing recovery function - careful with the regex to avoid malformed replacements
  if (content.includes('recoverFromError')) {
    // Find the start and end of the recoverFromError function
    const startMatch = content.match(/const\s+recoverFromError\s*=\s*useCallback\(\(\)\s*=>\s*\{/);
    const endMatch = content.match(/\}\s*,\s*\[\s*errorDetails\s*\]\s*\);/);
    
    if (startMatch && endMatch) {
      const startIndex = startMatch.index;
      const endIndex = endMatch.index + endMatch[0].length;
      
      // Replace only the function body, preserving any surrounding code
      content = content.substring(0, startIndex) + 
                enhancedRecoveryFunction + 
                content.substring(endIndex);
      
      console.log('Replaced existing recoverFromError function');
    } else {
      console.error('Could not find exact boundaries of recoverFromError function');
      return false;
    }
  } else {
  ...
  // Enhanced chunk error detection with similar careful matching
  ...
  // Replace the existing useEffect for chunk loading errors
  if (content.includes('Monitor for chunk loading errors')) {
    // Find the exact bounds of the useEffect for chunk loading errors
    const useEffectMatch = content.match(/\/\/\s*Monitor for chunk loading errors[\s\S]+?useEffect\(\(\)\s*=>\s*\{[\s\S]+?\}\s*,\s*\[[^\]]*\]\s*\);/);
    
    if (useEffectMatch) {
      content = content.replace(useEffectMatch[0], enhancedChunkErrorDetection);
    } else {
      console.error('Could not find exact boundaries of chunk error monitoring useEffect');
      return false;
    }
  } else {
