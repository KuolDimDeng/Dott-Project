...
const CognitoAttributes = {
  ...
  /**
   * Create a properly formatted attribute update object for Cognito
   * 
   * @param {Object} updates - Object with attribute names and values to update
   * @returns {Object} Formatted update object for Cognito API
   */
  createUpdateObject(updates) {
    const updateObject = {};
    
    for (const [name, value] of Object.entries(updates)) {
      // Validate the attribute before preparing the update
      const validation = this.validateAttribute(name, value);
      
      if (!validation.valid) {
        console.warn(`Invalid attribute update for ${name}:`, validation.errors);
        continue;
      }
      
      // Skip non-mutable attributes
      const rules = ValidationRules[name];
      if (rules && !rules.mutable) {
        console.warn(`Cannot update non-mutable attribute: ${name}`);
        continue;
      }
