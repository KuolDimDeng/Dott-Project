 ...
 * - Condition 11: Next.js version 15
...
const fileUtils = {
  ...
  async createBackup(filePath) {
    try {
      ...
      logger.error(`Failed to create backup for: ${filePath}`, { error: error.message });
      throw error;
    }
  }
};
...
async function main() {
  try {
    ...
    for (const file of CONFIG.targetFiles) {
      ...
      try {
        await fileUtils.createBackup(fullPath);
      } catch (error) {
        logger.warn(`Could not create backup for ${file}`, { error: error.message });
      }
    }
