 ...
 * 
 * This is a fixed version that corrects the path to tenantUtils.js.
 */
...
const TENANT_UTILS_PATH = '../src/utils/tenantUtils.js'; // Fixed path
...
try {
  ...
  const tenantUtilsPath = TENANT_UTILS_PATH;
...
`;
  ...
  if (scriptEntryPoint !== -1) {
    const newScriptEntry = `### Version0111_fix_post_signout_onboarding_redirect_fixed.mjs
- **Version**: 0111 v1.1
...
`;
  ...
  try {
    ...
    execSync('git add ./Version0111_fix_post_signout_onboarding_redirect_fixed.mjs');
    execSync('git add ./ONBOARDING_PERSISTENCE_FIX_SUMMARY.md');
    execSync('git add ./script_registry.md');
    
    execSync('git commit -m "Fix post-signout onboarding redirect issue (fixed script)"');
