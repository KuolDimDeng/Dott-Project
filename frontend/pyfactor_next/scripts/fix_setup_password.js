...
function fixSetupPasswordPage() {
  console.log(`Fixing syntax errors in ${SETUP_PASSWORD_PATH}`);
  ...
  const fileName = path.basename(SETUP_PASSWORD_PATH);
  const backupPath = path.join(backupDir, `${fileName}.backup-${timestamp}`);
  
  if (fs.existsSync(SETUP_PASSWORD_PATH)) {
    fs.copyFileSync(SETUP_PASSWORD_PATH, backupPath);
    console.log(`Created backup at ${backupPath}`);
  }
  
  // Create directory if it doesn't exist
  const dir = path.dirname(SETUP_PASSWORD_PATH);
...
export default function SetupPassword() {
  ...
  fs.writeFileSync(SETUP_PASSWORD_PATH, fixedContent);
  console.log(`Fixed syntax errors in ${SETUP_PASSWORD_PATH}`);
