...
/**
 * Standardized Tailwind loading spinner component
 * Based on the official Tailwind CSS spinner pattern
 */
const StandardSpinner = ({ 
  ...
  return (
    <div role="status" className={className}>
      ...
      <span className={showText ? "sr-only" : "sr-only"}>{text}</span>
    </div>
  );
};
...
export const ButtonSpinner = ({ text = 'Loading...' }) => {
  return (
    <>
      <StandardSpinner size="small" className="inline mr-2" />
      {text}
    </>
  );
};
