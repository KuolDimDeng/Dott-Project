...
export const Snackbar = ({
  ...
  // Get position from anchorOrigin
  ...
  const colorClass = colorClasses[severity] || colorClasses.info;
  
  // Default styles with extremely high z-index to ensure on top
  const defaultStyles = {
    zIndex: 9999,
    ...style
  };
  ...
  // Add animation
  const enterAnimation = 'animate-slideInUp';
  
  return (
    <div 
      className={`fixed ${positionClass} ${enterAnimation} ${className}`}
      style={defaultStyles}
      role="alert"
      aria-live="assertive"
    >
      <div className={`rounded-md shadow-lg border px-4 py-3 ${colorClass} max-w-md flex items-start`}>
        <div className="flex-grow">
          {/* Handle both string and React element messages */}
          {typeof message === 'string' ? (
            <p className="text-sm font-medium">{message}</p>
          ) : (
            message // Render React element directly
          )}
        </div>
        <button 
          onClick={onClose} 
          className="ml-4 text-white opacity-80 hover:opacity-100 focus:outline-none flex-shrink-0"
