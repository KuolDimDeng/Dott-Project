...
export default function TenantOnboardingBusinessInfoPage({ params }) {
  ...
  useEffect(() => {
    ...
    if (tenantId) {
      try {
        ...
        setTimeout(() => {
          router.push('/onboarding/business-info?from=tenant_redirect&ts=' + Date.now());
        }, 100);
      } catch (error) {
  ...
  return (
    <div className="flex h-screen items-center justify-center">
      <div className="text-center">
        ...
        <p className="mt-4 text-lg text-gray-600">Loading business information form...</p>
      </div>
    </div>
  );
} 