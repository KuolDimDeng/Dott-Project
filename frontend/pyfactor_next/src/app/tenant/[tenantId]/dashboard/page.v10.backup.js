  ...
  useEffect(() => {
    const initializeDashboard = async () => {
      try {
        ...
        try {
          // If we just came from sign-in, add extra parameters for better handling
          const authUrl = fromSignIn && sessionToken 
            ? `/api/auth/me?fromSignIn=true&token=${sessionToken}`
            : '/api/auth/me';
          
          // Check for pending session in sessionStorage
          const pendingSession = sessionStorage.getItem('pendingSession');
          const headers = {};
          if (pendingSession && fromSignIn) {
            headers['x-pending-session'] = pendingSession;
          }
          ...
          // If session is pending (cookies still propagating), wait and retry
          if (authData.sessionPending && fromSignIn) {
            logger.info('[TenantDashboard] Session pending, waiting for cookies to propagate...');
            await new Promise(resolve => setTimeout(resolve, authData.retryAfter || 1000));
            
            // Retry the authentication check
            const retryResponse = await fetch('/api/auth/me');
            if (retryResponse.ok) {
              const retryData = await retryResponse.json();
              if (retryData.authenticated && !retryData.sessionPending) {
                authData.user = retryData.user;
                authData.sessionPending = false;
              }
            }
          }
