...
export async function GET(request) {
  ...
  try {
    ...
    logger.info('[JobsDataCustomers] 👥 Session check:', { 
      hasSession: !!session, 
      sessionId: session?.sessionId 
    });

    if (!session?.token) {
      logger.error('[JobsDataCustomers] 👥 No session token found');
      return new Response(
    ...
    const backendUrl = `${process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.dottapps.com'}/api/jobs/data/customers/`;
