...
export async function GET(request) {
  try {
    ...
    const response = await fetch(`${BACKEND_URL}/api/hr/api/current-employee/`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[Current Employee] Get failed:', { status: response.status, error });
      return NextResponse.json({ error: 'Failed to get employee info' }, { status: response.status });
    }

    const data = await response.json();
    console.log('[Current Employee] Data retrieved:', data);
    return NextResponse.json(data);
  } catch (error) {
    console.error('[Current Employee] Get error:', error);
