...
const NavIcons = {
  ...
  Marketplace: (props) => ( // Keep for backward compatibility
    <svg className={props.className} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
    </svg>
  ),
...
const MainListItems = ({
  ...
  const menuItems = [
    ...
    */
    ...
    {
      icon: <NavIcons.Discover className="w-5 h-5" />,
      label: t('mainMenu.discover'),
      subItems: [
        { 
          label: t('subMenu.dashboard'), 
          onClick: (value) => {
            ...
            const payload = { 
              item: 'marketplace', 
              navigationKey,
              originalItem: 'Discover Dashboard'
            };
        ...
        { 
          label: t('subMenu.findBusinesses'), 
          onClick: (value) => {
            const navigationKey = `nav-${Date.now()}`;
            const payload = { 
              item: 'marketplace-search', 
              navigationKey,
              originalItem: 'Find Businesses'
            };
        ...
        { 
          label: t('subMenu.myOrders'), 
          onClick: (value) => {
            const navigationKey = `nav-${Date.now()}`;
            const payload = { 
              item: 'marketplace-orders', 
              navigationKey,
              originalItem: 'My Orders'
            };
        ...
        { 
          label: t('subMenu.myListing'), 
          onClick: (value) => {
            const navigationKey = `nav-${Date.now()}`;
            const payload = { 
              item: 'marketplace-listing', 
              navigationKey,
              originalItem: 'My Business Listing'
            };
        ...
        { 
          label: t('subMenu.messages'), 
          onClick: (value) => {
            const navigationKey = `nav-${Date.now()}`;
            const payload = { 
              item: 'chat', 
              navigationKey,
              originalItem: 'Messages'
            };
