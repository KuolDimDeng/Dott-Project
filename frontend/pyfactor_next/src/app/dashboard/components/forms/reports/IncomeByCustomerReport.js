...
const IncomeByCustomerReport = () => {
  ...
  const generateReport = async () => {
    try {
      setLoading(true);
      const params = {
        report_type: 'income_by_customer',
  ...
  const handleExport = async (format) => {
    try {
      ...
      link.href = url;
      link.setAttribute('download', `income_by_customer_${dateRange.startDate}_${dateRange.endDate}.${format}`);
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        <h1 className="text-2xl font-bold mb-4">Income by Customer Report</h1>
