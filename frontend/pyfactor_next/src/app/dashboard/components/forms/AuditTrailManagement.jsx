...
const AuditTrailManagement = () => {
  ...
  const applyFilters = () => {
    ...
    if (filters.user) {
      filtered = filtered.filter(trail => trail.user_name.toLowerCase().includes(filters.user.toLowerCase()));
    }
    if (filters.actionType) {
      filtered = filtered.filter(trail => trail.action_type === filters.actionType);
    }
    if (filters.transactionType) {
      filtered = filtered.filter(trail => trail.transaction_type.toLowerCase().includes(filters.transactionType.toLowerCase()));
    }
    ...
    <Box sx={{ width: '100%' }}>
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={4}>
          {selectedAuditTrail ? (
            <Paper sx={{ p: 2 }}>
              <Typography variant="h6" gutterBottom>Audit Trail Details</Typography>
              <Typography><strong>Date & Time:</strong> {new Date(selectedAuditTrail.date_time).toLocaleString()}</Typography>
              <Typography><strong>User:</strong> {selectedAuditTrail.user_name}</Typography>
              <Typography><strong>Action:</strong> {selectedAuditTrail.action_type}</Typography>
              <Typography><strong>Transaction ID:</strong> {selectedAuditTrail.transaction_id}</Typography>
              <Typography><strong>Transaction Type:</strong> {selectedAuditTrail.transaction_type}</Typography>
              <Typography><strong>Affected Accounts:</strong> {selectedAuditTrail.affected_accounts}</Typography>
              <Typography><strong>Old Value:</strong> {selectedAuditTrail.old_value}</Typography>
              <Typography><strong>New Value:</strong> {selectedAuditTrail.new_value}</Typography>
              <Typography><strong>Approval Status:</strong> {selectedAuditTrail.approval_status}</Typography>
              <Typography><strong>Notes:</strong> {selectedAuditTrail.notes}</Typography>
              <Typography><strong>IP Address:</strong> {selectedAuditTrail.ip_address}</Typography>
              <Typography><strong>Module:</strong> {selectedAuditTrail.module}</Typography>
            </Paper>
          ) : (
      ...
      <Box sx={{ mt: 4 }}>
        <Button variant="contained" color="primary" onClick={() => console.log('Export functionality to be implemented')}>
          Export Audit Trail
        </Button>
      </Box>
    </Box>
