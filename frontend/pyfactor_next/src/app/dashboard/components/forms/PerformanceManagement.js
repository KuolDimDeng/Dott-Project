 ...
 * Industry-standard employee performance management with CRUD operations and standard UI
 */
function PerformanceManagement({ onNavigate }) {
  ...
  const loadPerformanceRecords = async () => {
    try {
      setLoading(true);
      const data = await hrApi.performance.getAll();
      setPerformanceRecords(data || []);
    } catch (error) {
      logger.error('[PerformanceManagement] Error loading performance records:', error);
  ...
  const loadStats = async () => {
    try {
      const data = await hrApi.performance.getStats();
      setStats(data || stats);
    } catch (error) {
      logger.error('[PerformanceManagement] Error loading stats:', error);
    }
  };

  // Filter performance records based on search term
  const filteredRecords = useMemo(() => {
    if (!searchTerm) return performanceRecords;
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            ...
            <p className="text-3xl font-bold text-blue-600">{loading ? '-' : stats.totalReviews}</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <p className="text-gray-500 text-sm font-medium uppercase tracking-wide">Avg Score</p>
            <p className="text-3xl font-bold text-yellow-600">{loading ? '-' : stats.avgScore}/5</p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center">
          ...
          <div className="ml-4">
            <p className="text-gray-500 text-sm font-medium uppercase tracking-wide">Completed This Month</p>
            <p className="text-3xl font-bold text-green-600">{loading ? '-' : stats.completedThisMonth}</p>
          </div>
        </div>
      </div>
    </div>
  ...
  const renderTabNavigation = () => (
    <div className="border-b border-gray-200 mb-6">
      <nav className="flex space-x-8">
        {[
          { id: 'list', label: 'Performance Reviews', icon: ChartBarIcon },
          { id: 'create', label: 'Create Review', icon: DocumentTextIcon },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-black">Performance Management</h1>
          <p className="text-gray-600 mt-1">Manage employee performance reviews, goals, and evaluations</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="text-center py-12">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">Performance Management</h3>
          <p className="mt-1 text-sm text-gray-500">
            Full performance management functionality coming soon. This component follows the standard UI pattern.
          </p>
        </div>
      </div>
    </div>
