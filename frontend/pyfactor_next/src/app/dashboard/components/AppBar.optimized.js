...
*/
  ...
  const getBusinessNameFromCookies = () => {
    try {
      ...
      const alternateName =
        getCookie('custom:businessname') || getCookie('business_name');
      if (alternateName) {
        // Decode URL-encoded values (like %20 for spaces)
        const decodedName = decodeURIComponent(alternateName);
        logger.debug(
          '[AppBar] Found business name in alternate cookies:',
          alternateName,
          'decoded:',
          decodedName
        );
        ...
        try {
          ...
          if (
            ...
            setProfileData((prevData) => ({
              ...(prevData || {}),
              business_name:
                cookieBusinessName || storageBusinessName || 'Default Business',
              subscription_type: userData?.subscription_type || 'free',
              userData: {
                ...(prevData?.userData || {}),
                first_name: firstName || '',
  ...
  return (
    <>
      ...
      <header
        className="fixed top-0 left-0 right-0 h-16 bg-primary-main z-50 text-white border-b-2 border-primary-dark shadow-md"
                ...
                {displayInitials ||
                  (() => {
                    ...
                    if (firstName && lastName) {
                      ...
                      // Special handling for known email formats
                      if (email.toLowerCase().includes('kuoldimdeng')) {
                        // Extract K from 'kuol' and D from 'dimdeng'
                      ...
                      if (namePart.includes('.')) {
                        ...
                        // For emails without a period separator
                        // Check for specific name patterns first
                        if (namePart.toLowerCase().startsWith('kuol')) {
                          // Extract K from 'kuol' and D from 'dimdeng'
                          const firstPart = namePart.substring(0, 4); // kuol
                          const secondPart = namePart.substring(4); // dimdeng
                        ...
                        // For other cases, extract first and middle character
                        const midPoint = Math.floor(namePart.length / 2);
                        return `${namePart.charAt(0).toUpperCase()}${namePart.charAt(midPoint).toUpperCase()}`;
                      }
                      return email.charAt(0).toUpperCase();
                    } else {
                      return 'D';
                          ...
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-gray-500">
                              ...
                              {(() => {
                                ...
                                const subscription =
                                  profileData?.subscription_type ||
                                  businessData?.subscription_type ||
      ...
      {/* Add the create menu popup */}
      {showCreateMenu && (
        <>
          {/* Backdrop - only slightly visible */}
          <div
            className="fixed bg-black bg-opacity-10"
            onClick={handleClose}
            style={{
              position: 'fixed',
              top: 0,
          ...
          {/* Menu popup */}
          <div
            className="bg-white rounded-lg shadow-md p-4 w-72 max-h-[80vh] overflow-auto border border-gray-200"
            ...
            style={{
              ...
              left: drawerOpen
                ? '278px'
                : '78px' /* Position based on drawer state */,
              zIndex: 41,
          ...
          >
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-200">
              ...
              <button
                onClick={handleClose}
                className="text-gray-400 hover:text-gray-500 pointer-events-auto"
              >
                <svg
                  className="w-6 h-6"
                  fill="none"
            ...
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="flex flex-col items-center p-4 border border-gray-200 rounded-lg hover:bg-blue-50 pointer-events-auto"
              >
                <svg
