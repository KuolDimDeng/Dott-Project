...
export const useOnboarding = (formMethods) => {
  ...
  const validateStepData = useCallback((step, data) => {
    ...
    if (step === APP_CONFIG.onboarding.steps.PLAN) {
      ...
      const validBillingCycles = APP_CONFIG.app?.plans?.validBillingCycles || ['monthly', 'annual'];
  ...
  const saveStepWithRetry = useCallback(async (step, data, retryCount = 0) => {
    ...
    try {
      ...
      const submissionData = typeof data === 'object' ? data : {};

      logger.debug('Processing submission with data:', {
        step,
        data: submissionData,
        attempt: retryCount + 1
      });

      const validatedData = await validateStepData(step, submissionData);
      ...
      logger.debug('Save operation successful:', { step, result });
      return result;
  ...
  const initialize = useCallback(async () => {
      ...
      try {
        ...
        const stepValidation = steps.reduce((acc, step, index) => ({
          ...acc,
