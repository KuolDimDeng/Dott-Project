...
if (!appCache.getAll() || Object.keys(appCache.getAll()).length === 0) {
  ...
  appCache.set('tenant', {});
      ...
      try {
        // Auth0 equivalent - replaced Amplify import
// Previously: // Auth0 equivalent - replaced Amplify import
...
const session = await getAuth0Session();
                  ...
                  if (session && session.tokens) {
                    ...
                    sessionStorage.setItem('idToken', session?.id_token || session?.idToken);
                    ...
                    if (!appCache.get('auth')) appCache.set('auth', {});
                    appCache.set('auth.idToken', session?.id_token || session?.idToken);
                    appCache.set('auth.token', session?.id_token || session?.idToken);
