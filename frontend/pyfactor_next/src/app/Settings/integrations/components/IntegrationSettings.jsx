...
const IntegrationSettings = React.memo(({ initialStatus, initialPlatform, initialBusinessData, title }) => {
  ...
  const initiateShopifyOAuth = async () => {
    ...
    try {
      ...
      addMessage(error.response?.data?.message || 'An error occurred while initiating Shopify OAuth', 'error');
    } finally {
      setConnecting(false);
    }
  };
            ...
            <Box mt={3}>
              {currentPlatform === 'woocommerce' && (
                <>
                  ...
                  <Typography variant="body1" mt={1}>Connect to your WooCommerce store here.</Typography>
              ...
              {currentPlatform === 'shopify' && (
                <>
                  ...
                  <Typography variant="body1" mt={1}>Connect to your Shopify store here.</Typography>
                  <Box component="form" noValidate autoComplete="off" mt={2}>
                    <TextField
                      fullWidth
                      margin="normal"
