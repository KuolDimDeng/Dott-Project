...
import BillFiltersPanel from './BillFiltersPanel';
...
const BillsList = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await BillService.bulkDeleteBills(selectedBills);
      if (response.success) {
        toast.success(`${selectedBills.length} bills deleted successfully`);
        setSelectedBills([]);
        fetchBills();
        fetchStats();
      } else {
        toast.error(response.error || 'Failed to delete bills');
      }
    } catch (error) {
  ...
  const handleMarkAsPaid = async (billId) => {
    try {
      const response = await BillService.markBillAsPaid(billId);
      if (response.success) {
          ...
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1">
              <div className="relative">
                <input
                  type="text"
                  placeholder="Search bills by number, vendor, or PO number..."
