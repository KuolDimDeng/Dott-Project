...
import ServiceStatsWidget from './ServiceStatsWidget';
...
const ServicesList = () => {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      const response = await ServiceService.bulkDeleteServices(selectedServices);
      if (response.success) {
        ...
        toast.error(response.error || 'Failed to delete services');
      }
    } catch (error) {
  ...
  return (
    <div className="h-full flex flex-col bg-gray-50">
      ...
      {/* Stats Widget */}
      <div className="px-4 sm:px-6 lg:px-8 py-4">
        <ServiceStatsWidget stats={stats} />
      </div>
      ...
      {showDetailDialog && selectedService && (
        <ServiceDetailDialog
