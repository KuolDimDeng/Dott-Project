...
# Version0065_final_solution_stack_deploy.sh
# Version: 1.0.0
# Date: 2025-05-22
# Author: AI Assistant
# Purpose: Create a final simplified deployment with solution stack parameter
...
echo -e "${BLUE}===== DottApps Final Solution Stack Deployment (v1.0.0) =====${NC}"
...
SOLUTION_STACK="64bit Amazon Linux 2 v3.5.2 running Docker"
...
echo -e "${BLUE}Checking available solution stacks...${NC}"
STACKS=$(aws elasticbeanstalk list-available-solution-stacks --query "SolutionStacks[?contains(@, 'Docker')]" --output text | head -n 5)

echo -e "${YELLOW}Available Docker solution stacks:${NC}"
echo "$STACKS" | nl
